...
# UNLIKE CHASE_2026_07_LINES above, this mirrors the real line order/wording
# of an actual JPMorgan Chase Bank, N.A. checking statement (confirmed by
# inspecting one — no real name/address/account number reproduced here, only
# its structure). Two things this fixture exists specifically to prove:
# 1. pypdf's text extraction for Chase's multi-column layout (address block +
#    "CUSTOMER SERVICE INFORMATION" sidebar) genuinely separates the account
#    number from its "Account Number:" label — the digits appear standalone,
#    near the very top, well before the label reappears later in reading
#    order. try_parse_chase() correctly returns None on this (it never finds
#    an adjacent label+digits pair) and defers to the LLM — this is NOT a
#    parser bug to "fix" by guessing at position; it's the deterministic
#    tier correctly declining a layout it can't confidently handle.
# 2. The account holder's name has no title prefix and no "Customer Name:"
#    label at all — just a plain line, unlike every Indian-bank fixture here.
CHASE_REAL_LAYOUT_2026_07_LINES = [
    "000000123456789",
    "CUSTOMER SERVICE INFORMATION",
    "06024080101000000021",
    "Page 1 of 2",
    "AMOUNT",
    "DATE DESCRIPTION AMOUNT BALANCE",
    "SM",
    "00602408 DRE 703 219 02426 NNNNNNNNNNN 1 000000000 15 0000",
    ...
    "07/16 Sample ACH Payment M0000 Web ID: 0000000000 -150.00 2,650.00",
    "CHECKING SUMMARY",
    "TRANSACTION DETAIL",
]
...
# Used only by test_hdfc_joint_account_keeps_both_names (parser-correctness unit
# test — investor_name must keep both holders, not just the first) — no PDF is
# built from these lines since no eval case references them.
...
def generate_all() -> None:
    ...
    _write_encrypted(
        CHASE_REAL_LAYOUT_2026_07_LINES,
        TEST_PDF_PASSWORD,
        FIXTURES_DIR / "chase_real_layout_2026_07.pdf",
    )
